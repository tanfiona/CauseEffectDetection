...
class DefaultDataCollator(DataCollator):
    ...
    def collate_batch(self, features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        # Add if graph data exists
        if hasattr(first, "graph_data") and first.graph_data is not None:
            multi_graphs = [f.graph_data for f in features]
            batch["graph_data"] = Batch.from_data_list(multi_graphs)

        # Handling of all other possible attributes.
        # Again, we will use the first element to figure out which key/values are not None for this model.
        for k, v in vars(first).items():
            if k not in ("label", "label_ids", "graph_data") and \
