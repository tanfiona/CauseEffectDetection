...
class ViterbiDecoder():
    ...
    def __init__(self, tag_map):
        ...
        self.start_tag = tag_map['_']
        self.end_tag = tag_map['_']
    ...
    def decode(self, scores, lengths):
        ...
        backpointers = torch.ones((batch_size, word_pad_len, self.tagset_size), dtype=torch.long) * self.end_tag

        for i in range(batch_size):
            for t in range(lengths[i]):
                if t == 0:
                    scores_upto_t[i] = scores[i, t, self.start_tag, :]
                    backpointers[i, t, :] = torch.ones((1, self.tagset_size),dtype=torch.long) * self.start_tag
                else:
                    scores_upto_t[i], backpointers[i, t, :] = torch.max(
                        scores[i, t, :, :] + scores_upto_t[i].unsqueeze(1),
                        dim=0)
        ...
        final_scores = torch.zeros(batch_size, word_pad_len, self.tagset_size).scatter_(2, decoded.unsqueeze(2), 1)
