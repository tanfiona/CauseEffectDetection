...
class PretrainedConfig(object):
    ...
    def __init__(self, **kwargs):
        ...
        # For POS labels
        self.id2poslabel = kwargs.pop("id2poslabel", None)
        if self.id2poslabel is not None:
            self.id2poslabel = dict((int(key), value) for key, value in self.id2poslabel.items())
            # Keys are always strings in JSON so convert ids to int here.
        else:
            self.num_pos_labels = kwargs.pop("num_pos_labels", 2)
        
        # For whether to add features and layers
        self.add_pos = kwargs.pop("add_pos", False)
        self.use_bilstm = kwargs.pop("use_bilstm", False)

        self.use_graph = kwargs.pop("use_graph", False)
        self.graph_purpose = kwargs.pop("graph_purpose", 'classifier')
        self.use_graphlstm = kwargs.pop("use_graphlstm", False)
        self.graph_hidden = kwargs.pop("graph_hidden", 512)
        self.graph_out = kwargs.pop("graph_hidden", 256)
        self.add_feats = kwargs.pop("add_feats", False)

        # Path to transition matrix for Viterbi 
        self.transitions_path = kwargs.pop("transitions_path", None)
